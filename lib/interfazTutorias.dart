...
class InterfazTutorias extends StatefulWidget {
  ...
  InterfazTutorias({required this.userId});
...
class _InterfazTutoriasState extends State<InterfazTutorias> {
  ...
  Future<void> _fetchTutorias() async {
    final response = await http.get(Uri.parse('http://localhost/tutormeup/obtener_tutorias.php?user_id=${widget.userId}'));
      ...
      builder: (context) {
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                _registerTutoria(tituloController.text, descripcionController.text);
                Navigator.of(context).pop();
              },
